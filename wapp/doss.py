...
def snapshot(root, name, module_names):
    # XXX raises an exception in Windows if oldsnapshot exists
    #
    # Write a snapshot of the root object to a temporary file.
    fd, filename = tempfile.mkstemp()
    #   (You'd think the following line would be the right way to
    #   get a file object open on the temporary file, but Python 
    #   complains: IOError: [Errno 9] Bad file descriptor)
    #   f = os.fdopen(fd)
    f = file(filename, 'w')
    serialize(root, module_names, f)
    # Back up the previous current snapshot, if any.
    try:
        os.rename(name + '.py', 'old' + name)
    except OSError, e:
        print 'Warning: failed to rename old snapshot: %s' % e
    # Make what we just wrote be the new current snapshot.
    os.rename(filename, name + '.py')


# So we can serialize datetime values.
import datetime

def datetime_uneval(self, context, label):
    return context.uncall('datetime.datetime',
			  year=self.year,
			  month=self.month,
			  day=self.day,
			  hour=self.hour,
			  minute=self.minute,
			  second=self.second,
			  microsecond=self.microsecond,	# XXX skip?
			  tzinfo=self.tzinfo)

# Ditto for  struct_time values
# (N.B. they get resurrected as tuples instead, which should be OK):
import time			
...
def serialize(object, module_names, file):
    depiction = Context().depict(object)
    # XXX the following line imposes the requirement that object be an instance
    depiction.refcount += 1	# ensure it goes in the defs table
    defs = Defs()
    walked = defs.walk_until_quiescent(depiction)
    out = Output(file)
    print_imports(out, module_names)
    out.pprint(defs)
    defs.pp_effects(out)
    out.pprint(walked)
    out.emit('root = ')
...
def print_imports(out, module_names):
    for m in module_names:     # XXX should compute them automatically
	out.emit('import %s\n' % m)
    out.newline()
...
class Context:		      # XXX what's a better name than context?
	...
	if type(object) == datetime.datetime:
	    label.referent = datetime_uneval(object, self, label)
	else:
...
simple_data_types = map(type, [0, 0.0, 0L, '', u'', True, None, time.gmtime()])
...
class Output:
    ...
    def tab(self):
        # This code is tuned because it actually matters for
        # speed (and size of the output -- '\t' makes a difference
        # there).
        # 
        # N.B. For very large outputs it would be profitable to
        # write nothing instead (turn this method into a no-op).
        # Currently I still care more about nice easy-to-understand
        # output.
        assert self.column == 0 or self.column == self.margin
        d = self.margin - self.column
        self.stream.write('\t' * (d // 8) + ' ' * (d % 8))
        self.column += d
